...
def load_data():
    ...
    exclude_cols = ['city', 'year', 'data_type', 'env_score', 'pred_lr', 'pred_rf']
    for col in df.columns:
        if col not in exclude_cols and not col.startswith('unnamed'):
            df[col] = pd.to_numeric(df[col], errors='coerce')

    # Рассчитываем базовый env_score, если его нет в явном виде
    if 'env_score' not in df.columns:
        if 'net_salary' in df.columns and 'people' in df.columns:
            df['env_score'] = df['net_salary'].rank(pct=True) * 0.4 + df['people'].rank(pct=True) * 0.3
        else:
            df['env_score'] = 0.5
...
with t3:
    ...
    with col_e:
        sim_city = st.selectbox("Выберите город для моделирования:", sorted(latest_df['city'].unique()), key='sim_tab3')
        sim_feat = st.selectbox("Изменить показатель:", feature_columns, format_func=lambda x: readable_metrics.get(x, x))
        
        city_rows = latest_df[latest_df['city'] == sim_city]
        if not city_rows.empty:
            city_data = city_rows.iloc[0]
            curr_val = float(city_data[sim_feat]) if pd.notna(city_data[sim_feat]) else 0.0
            
            st.write(f"Текущее значение ({int(latest_year_num)} г.): **{curr_val:,.2f}**")
            change = st.slider("Сценарий изменения показателя (%):", -50, 50, 0, key='slider_sim')
            new_val = curr_val * (1 + change/100)
            st.write(f"Моделируемое значение на 2025 г.: **{new_val:,.2f}**")
        else:
            st.warning("Нет базовых исторических данных для симуляции по данному городу.")
            change = 0
    ...
    with col_f:
        ...
        base_score = float(city_data['env_score']) if ('city_data' in locals() and pd.notna(city_data['env_score'])) else 0.5
        
        simulated_score = base_score + (change / 100.0) * weight
        simulated_score = max(0.0, min(1.0, simulated_score)) # Ограничение границ [0, 1]
        
        # Исправленный блок графика спидометра
        ...
        fig_indicator.update_layout(height=350, margin=dict(t=50, b=20, l=20, r=20))
