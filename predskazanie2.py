...
# Дополнительно переименовываем проблемные колонки (если нужно)
df.rename(columns={
    'avg_age': 'avg_age',
...
# ------------------------------
# 2. Список признаков (все числовые, кроме city, year)
# ------------------------------
feature_columns = [
...
# ------------------------------
# 3. Интерполяция и заполнение пропусков (с сохранением последнего значения)
# ------------------------------
def interpolate_and_fill(df, feature_name):
    ...
    # Заполнение медианой по городу (только положительные для non_zero_features)
    ...
    # Глобальная медиана (или минимальное положительное) для оставшихся NaN
    if feature_name in non_zero_features:
...
# Сохраняем датасет ПОСЛЕ ИНТЕРПОЛЯЦИИ (без нормализации)
...
# ------------------------------
# 4. Нормализация и интегральный показатель (env_score) – равные веса
# ------------------------------
scalers = {}
norm_cols = []
for col in feature_columns:
...
# env_score = среднее арифметическое нормализованных признаков (равные веса)
df_full['env_score'] = df_full[norm_cols].mean(axis=1)

# ------------------------------
# 5. Создание лагов (признаки за предыдущий год)
# ------------------------------
...
# ------------------------------
# 6. Разбиение 80% / 20% (случайное)
# ------------------------------
X = df_lagged[lag_cols]
y = df_lagged['env_score']

X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.2, random_state=222, shuffle=True
)
...
# ------------------------------
# 7. Обучение моделей и оценка качества
# ------------------------------
print("\n=== ЛИНЕЙНАЯ РЕГРЕССИЯ ===")
lr = LinearRegression()
...
# ------------------------------
# 8. Итоговая статистика по обработанному датасету
# ------------------------------
...
imp = pd.DataFrame({'Признак': lag_cols, 'Важность': rf.feature_importances_}).sort_values('Важность', ascending=False)
print(imp.head(10).to_string(index=False))

# ------------------------------
# 11. Вывод примера таблиц после интерполяции и после лагов
# ------------------------------
print("\n=== ПРИМЕР ДАННЫХ ПОСЛЕ ИНТЕРПОЛЯЦИИ (первые 10 строк) ===")
print(df_full[['city', 'year'] + feature_columns].head(10).to_string(index=False))

print("\n=== ПРИМЕР ДАННЫХ ПОСЛЕ ЛАГОВ (первые 10 строк) ===")
print(df_lagged[['city', 'year'] + norm_cols[:3] + ['env_score'] + lag_cols[:3]].head(10).to_string(index=False))